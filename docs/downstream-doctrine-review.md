...
This is the direct answer to §1.3. Have `scripts/install.{sh,ps1}` write a manifest of
what they copied — path + sha256 of the source — and add a preflight check that hashes
the installed copies and reports the ones that differ from the current checkout. That
turns "re-run `pteam install`, probably" into a check that names the three stale files.
It also makes the existing `policy-core` load check strictly stronger rather than
replacing it: loads *and* is the version this CLI enforces.
