...
echo "  2. Install the MCP adapter (PINNED version — Paseo tools depend on it):"
echo "     pi install npm:pi-mcp-adapter@2.19.0"
echo "  3. Merge config/paseo.providers.example.json into ~/.paseo/config.json"
echo "     (agents.providers.pi-* + daemon.mcp.injectIntoAgents: true)."
echo "  4. Copy config/model-routing.example.json to ~/.paseo-pi-team/model-routing.local.json"
...
echo "  5. Restart the Paseo daemon (kills running agents — do it when ready)."
echo "  6. In pi, run /reload to load the new extension, then /team-role."
echo "  7. Verify host readiness (repo-root independent):"
