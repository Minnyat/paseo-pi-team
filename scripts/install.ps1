...
Write-Host "  2. Install the MCP adapter (PINNED version - Paseo tools depend on it):"
Write-Host "     pi install npm:pi-mcp-adapter@2.19.0"
Write-Host "  3. Merge config/paseo.providers.example.json into ~/.paseo/config.json"
Write-Host "     (agents.providers.pi-* + daemon.mcp.injectIntoAgents: true)."
Write-Host "  4. Copy config/model-routing.example.json to ~/.paseo-pi-team/model-routing.local.json"
...
Write-Host "  5. Restart the Paseo daemon (kills running agents - do it when ready)."
Write-Host "  6. In pi, run /reload to load the new extension, then /team-role."
Write-Host "  7. Verify host readiness (repo-root independent):"
