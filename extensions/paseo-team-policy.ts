...
export function detectRole(): TeamRole | undefined {
  ...
  return raw === "supervisor" || raw === "lead" || raw === "peer" ? raw : undefined;
}
...
export function policyFor(role: TeamRole, peerMode: PeerMode): Policy {
  switch (role) {
    case "lead":
      return { allow: [...PI_WRITE, ...ALL_PASEO_TOOLS], deny: [] };
    case "supervisor":
      return {
        allow: ["read", ...PASEO_TOOLS.monitoring, "send_agent_prompt"],
        deny: [
          "write",
          "edit",
          ...PASEO_TOOLS.discovery,
    ...
    case "peer":
      return peerMode === "write"
        ? { allow: [...PI_WRITE], deny: [...ALL_PASEO_TOOLS] }
        : { allow: [...PI_READ_ONLY], deny: [...ALL_PASEO_TOOLS, "write", "edit"] };
  }
}

export function denyReason(role: TeamRole, peerMode: PeerMode, toolName: string): string {
  if (role === "peer" && ALL_PASEO_TOOLS.includes(toolName)) {
    return "Peer cannot orchestrate agents or manage workspaces. Report a DEPENDENCY_REQUEST to the Lead instead.";
  }
  if (role === "peer" && peerMode !== "write" && (toolName === "write" || toolName === "edit")) {
...
function applyPolicy(pi: ExtensionAPI, r: TeamRole): Policy {
  ...
  const allowed = [...new Set([...policy.allow, ...extraTools()])].filter((name) =>
    registered.has(name),
  );
...
function registerDebugCommands(pi: ExtensionAPI, r: TeamRole | undefined) {
  pi.registerCommand("team-role", {
    ...
    handler: async (_args, ctx) => {
      if (!r) {
        ctx.ui.notify("PASEO_PI_ROLE is unset — extension is passive (no restrictions).", "warning");
        return;
      }
      const p = policyFor(r, peerMode);
      ctx.ui.notify(`role=${r} peerMode=${peerMode}\n${describePolicy(p)}`, "info");
    },
  });
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event) => {
    ...
    if (policy.deny.includes(event.toolName)) {
      return { block: true, reason: denyReason(role, peerMode, event.toolName) };
    }
